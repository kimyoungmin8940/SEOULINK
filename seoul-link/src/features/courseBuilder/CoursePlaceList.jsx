        ...
        <section className="course-builder-selected-area">
            <div className="course-builder-course-header">
                <div>
                    <h2>나만의 서울 코스</h2>
                    <p>
                        DAY {activeDayNo} - {dayPlaces.length}개 장소
                    </p>
                </div>

                <div className="course-builder-day-navigation" aria-label="여행 일차 이동">
                    <button
                        type="button"
                        onClick={onPreviousDay}
                        disabled={activeDayNo <= 1}
                        aria-label="이전 일차"
                    >
                    ...
                    <button
                        type="button"
                        onClick={onNextDay}
                        disabled={activeDayNo >= totalDayCount}
                        aria-label="다음 일차"
                    >
            ...
            <div className="course-builder-course-list">
                ...
                {dayPlaces.map(({ place, globalIndex }, dayIndex) => {
                    ...
                    const nextDayItem = dayPlaces[dayIndex + 1];
                    const hasNextPlace = Boolean(nextDayItem);
                        ...
                        <div key={placeKey} className="course-builder-course-segment">
                            <article className="course-builder-selected-card">
                                <div className="course-builder-place-card-top">
                                    ...
                                    <button
                                        type="button"
                                        className="course-builder-remove-course-place"
                                        onClick={() => onRemovePlaceFromCourse(placeKey)}
                                ...
                                <div className="course-builder-card-controls">
                                    <label className="course-builder-stay-field">
                                        ...
                                        <input
                                            ...
                                            onChange={(event) => onUpdateCoursePlaceField(globalIndex, "stayMinutes", event.target.value)}
                                    ...
                                    <div className="course-builder-order-buttons">
                                        <button
                                            type="button"
                                            onClick={() => onMoveCoursePlace(globalIndex, -1)}
                                            disabled={dayIndex === 0}
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => onMoveCoursePlace(globalIndex, 1)}
                                            disabled={dayIndex === dayPlaces.length - 1}
                                ...
                                {place.routeStatusMessage && (
                                    <p className="course-builder-route-error">경로 계산 안내: {place.routeStatusMessage}</p>
                                )}
                            </article>

                            {hasNextPlace && (
                                <div className="course-builder-route-info">
                                    <Car size={15} aria-hidden="true" />
                                    <span>
                                        차량 {formatDurationMinute(place.moveDurationMin)} · {formatDistanceMeter(place.moveDistanceM)}
            ...
            <div className="course-builder-time-summary" aria-live="polite">
                <div className="course-builder-time-summary-title">
                    <Clock3 size={20} aria-hidden="true" />
                    <span>예상 시간</span>
                    <strong>{formatDurationMinute(dayTimeSummary.totalMinutes)}</strong>
                </div>
                <p>
                    체류 {formatDurationMinute(dayTimeSummary.stayTotalMinutes)} + 차량 이동 {formatDurationMinute(dayTimeSummary.moveTotalMinutes)}
                </p>
            </div>
